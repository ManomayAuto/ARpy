...
if curdate == '1':
    ...
    repdate = datetime.today().strftime('%m-%Y')
    wb.save('C:/Mantra/Reports/Consolidatedactivity/ConsolidatedActivityReportt_'+dt+'.xlsx')

else:
    dt = (datetime.today()).strftime('%m-%Y')
    print(dt)
    t = os.path.exists('C:/Mantra/Reports/Consolidatedactivity/ConsolidatedActivityReportt_'+dt+'.xlsx')
    print(t)

    if t:
        book = xlrd.open_workbook('C:/Mantra/Reports/Consolidatedactivity/ConsolidatedActivityReportt_'+dt+'.xlsx')
        sheet = book.sheet_by_index(0)
        nr = sheet.nrows
        print(nr)
        wb = load_workbook('C:/Mantra/Reports/Consolidatedactivity/ConsolidatedActivityReportt_'+dt+'.xlsx')
        ws = wb["Sheet"]
        row = nr + 1
