...
#select all the coverage types and display them on the header of excel file
cur = conn.cursor()
select_query = "select Description from CoverType"
...
file_name = 'C:/Mantra/Reports/QuoteReports/quotebycoverage/''quotebycoverage'+'_'+t_month_year+'.xlsx'
wb = Workbook()
ws = wb.active
ws.append(header1)
...
cur1 = conn.cursor()
select_query1= "SELECT u.Branch, u.Name, q.TypeofCover, count(q.TypeofCover) FROM Users_man as u LEFT JOIN QuoteTransaction as q ON u.Name = q.InitiatedUsername where u.Branch = ? and u.Permissions LIKE ? and  month(QuoteIssuanceDate) = MONTH(CURRENT_TIMESTAMP) GROUP BY u.Branch, u.Name, q.TypeofCover union SELECT u.Branch, u.Name, q.TypeofCover, count(q.TypeofCover) FROM Users_man as u LEFT JOIN QuoteTransaction as q ON u.Name = q.InitiatedUsername where u.Branch = ? and u.Permissions LIKE ? GROUP BY u.Branch, u.Name, q.TypeofCover "
...
if row:
    ...
    #print each user and the corresponding coverages on the excel
    count = 0
