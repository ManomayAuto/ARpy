...
t = os.path.exists('C:/Mantra/Reports/Expiredpolicies/Expiredpolicies.xlsx')
print(t)

if t:
    book=xlrd.open_workbook('C:/Mantra/Reports/Expiredpolicies/Expiredpolicies.xlsx')
    sheet=book.sheet_by_index(0)
    nr= sheet.nrows
    print(nr)
    wb = load_workbook("C:/Mantra/Reports/Expiredpolicies/Expiredpolicies.xlsx")
    ws = wb["Sheet"]
    row = nr + 1
else:
    wb = Workbook()
    ...
    ws.cell(1, 8).number_format = '0.00'
...
print(dt)
csr = conn.cursor()
csr.execute("Select Clientname,Clientid,EmailId,PhnNum1,PhnNum2,PhnNum3,PolicyNum,PremDue,EquityDate,Branch,AccEXe from ae  where DTE between -30 and -1  and ReductionCheckBox = 0 order by Branch")
rcrds = csr.fetchall()
print(rcrds)
...
for r in (rcrds):
    ...
    ws.cell(row, col+7).value = float(r[7])
...
csr = conn.cursor()
csr.execute("Select Clientname,clientid,EmailId,PhnNum1,PhnNum2,PhnNum3,PolicyNum,PremDue,EquityDate,Branch from nonae where  DTE between -30 and -1 and ReductionCheckBox = 0 order by branch")
rcrds = csr.fetchall()
print(rcrds)
if AErecords == 0:
    row = 1
...
wb.save("C:/Mantra/Reports/Expiredpolicies/Expiredpolicies.xlsx")
