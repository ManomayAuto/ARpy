...
def AEEscalation():
    ...
    #print(filetype())
    print("asd")
    # t = threading.Thread(target=delete_excel, args=(file_name,))
    # t.start()
    #     # t = threading.Thread(target=os.remove, args=(file_name,))
    #     # t.start()
...
def ExpiredPolicies():
    ...
    file_name = "C:/Mantra/Reports/Expiredpolicies/Expiredpolicies.xlsx"
    print(file_name)
    return send_file(file_name)
...
def NONAEEscalation():
    ...
    workbook = xlsxwriter.Workbook('C:/Mantra/TempFolder/''NONAEEscalationReport' + '_' + dt + '.xlsx')
    worksheet = workbook.add_worksheet()
